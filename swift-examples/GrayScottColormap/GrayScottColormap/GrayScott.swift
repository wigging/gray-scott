...
class GrayScott: ObservableObject {
    ...
    var nt = 10_000  // total number of time steps
        
    func runSimulation(F: Float, k: Float) async -> [Float] {

        let Du: Float = 0.2    // diffusion coefficient for U
        let Dv: Float = 0.1    // diffusion coefficient for V
        
        let n = 300            // grid size n x n, try a value of 128 or 256
        ...
        let uNorm = U.normalize()
        print("Min U is \(uNorm.min()!)")
        print("Max U is \(uNorm.max()!)")
        print("Done.")
